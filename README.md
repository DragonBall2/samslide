...
- **핸드오프 노트(필독)**: [`docs/handoff.md`](docs/handoff.md) — 현재 상태, 다음 할 일, 주의할 함정
...
> 이 프로젝트는 여러 개발 환경/에이전트 간 git 으로 핸드오프된다. 새 체크아웃에서 작업을 시작하거나, 세션을 끝맺을 때는 `docs/handoff.md` 를 반드시 확인·갱신한다.
...
- ⏳ DB 미도입 (인메모리 저장, API 재시작 시 데이터 소실) — M1 후반 예정
