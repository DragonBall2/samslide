...
- http://localhost:3000/presentations — 프레젠테이션 관리 UI
...
- `@samslide/types` — Zod 슬라이드 스키마 6종(`content`, `multiple_choice`, `true_false`, `short_answer`, `word_cloud`, `qna`) + `Presentation` + `@samslide/types/api` 서브경로
...
### M1 진행 중
- **Presentation CRUD** — 인메모리 저장소(`PresentationsRepository`). 엔드포인트: `GET/POST /presentations`, `GET/PATCH/DELETE /presentations/:id`.
- **Slide CRUD + Reorder** — `POST /presentations/:id/slides`, `PATCH/DELETE /presentations/:id/slides/:slideId`, `POST /presentations/:id/slides/reorder`. 6종 kind 모두 지원.
- Zod 기반 `ZodValidationPipe` 범용 검증. `applyCrossFieldValidation`으로 `SlideSchema` ↔ `CreateSlideRequestSchema` 가 동일한 교차검증 공유.
- web-host `/presentations` (리스트·생성·삭제), `/presentations/[id]` (슬라이드 에디터: 6종 폼 + 추가/편집/삭제/순서변경).
- **용어**: 한국어 UI는 "프레젠테이션", 코드/API/타입/URL 모두 `Presentation` / `/presentations`. 과거 `Deck`/`/decks` 명칭은 전부 리네임됨 (git log 참조).
...
- [ ] `prisma/schema.prisma` 작성 — `presentations`, `slides` 테이블 (slide는 `kind`, `order`, `config_json` JSONB 컬럼), `media_assets` 는 나중에
- [ ] `PresentationsRepository` 를 인터페이스로 추출(`InMemoryPresentationsRepository`, `PrismaPresentationsRepository` 두 구현)
- [ ] `PresentationsModule` 이 환경 변수로 구현 선택 (`DATABASE_URL` 존재 시 Prisma, 없으면 인메모리)
...
- [ ] 프레젠테이션 제목·설명 인라인 편집 (현재는 생성 시에만 설정 가능)
- [ ] 슬라이드 폼 클라이언트측 유효성 검증 강화 (정답 없는 객관식 막기 등 — 현재는 서버 400에 의존)

### 3.3 M2 예고 (실시간 세션)
- `PresentationsRepository` 인터페이스를 추출하고 `InMemoryPresentationsRepository` / `PrismaPresentationsRepository` 두 구현 제공.
...
| 에디터 API 는 **슬라이드 단위 엔드포인트** | 프레젠테이션 전체 교체식으로 만들면 DB 전환 시 UX 재작업 필요 | 이 문서 §3.1 |
